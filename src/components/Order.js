...
const Order = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        const response = await axios.get(`https://outlier-and-da-backend.onrender.com/api/users/orders/${userId}`, {
          headers: {
        ...
        const fetchedOrders = await Promise.all(response.data.orders.map(async (order) => {
          try {
            const orderResponse = await axios.get(`https://outlier-and-da-backend.onrender.com/api/orders/${order.orderId}`, {
  ...
  const handleDelete = async (orderId) => {
    ...
    try {
      ...
      await axios.delete(`https://outlier-and-da-backend.onrender.com/api/users/orders/${orderId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleViewDetails = async (orderId) => {
    ...
    try {
      ...
      const response = await axios.get(`https://outlier-and-da-backend.onrender.com/api/orders/${orderId}/${userId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
